...
void DecodeSciRaw(
 ...
 const string NameRoot
  ...
  map< int, Sensor > sensorMap = LoadSensorMap(1);
  ...
  ofstream ofSyncData(Form("%s/SyncData.dat",NameRoot.data())); //open decode Sci txt data
  ...
  //data variable
  int Word, SeqSAD, UTCDay, UTCHour, UTCMin, UTCSec, UTCSecS;
  bool GTMID = false;
  ...
  while(inRawSci>>Hitdata[0]>>Hitdata[1]>>Hitdata[2]){ // Get 3 byte data for #Word 0
    ...
    if(Hitdata[0]=="CA"){//Header (0xCA) - Get Header of Sync. data 
      ...
      // cout<<Hitdata[0]<<Hitdata[1]<<Hitdata[2]<<" ";
      inRawSci>>Hitdata[0]>>Hitdata[1]>>Hitdata[2];//#Word 2
      Word = GetWordVal(Hitdata);
      ...
      inRawSci>>Hitdata[0]>>Hitdata[1]>>Hitdata[2]; Word=GetWordVal(Hitdata); 
      ...
      for(int iByte=0;iByte<6;iByte++) inRawSci>>Hitdata[0];
      
      inRawSci>>Hitdata[0]>>Hitdata[1]>>Hitdata[2];
      Word = GetWordVal(Hitdata);
      LiveTimeA = GetBits(Word,12,23,5);//#Word 10 07-0 -> Word 11 23-20
      LiveTimeB = GetBits(Word, 0,11,5);//#Word 11 19-8
      
      inRawSci>>Hitdata[0]>>Hitdata[1]>>Hitdata[2];//#Word 
      Word = GetWordVal(Hitdata);
      ...
      for(int iWord=13;iWord<=14;iWord++){//#Word 4->#Word 14
        inRawSci>>Hitdata[0]>>Hitdata[1]>>Hitdata[2];//skip #Word 3.1->14.3
      } 
      ofSyncData<<"\n A Sync. data:        --------------\n";
      ...
      if     (((Word>>23)&1) == 0){// cout<<"<---Find Header \"0\" for Event-Hit data"<<endl;
        ...
        // if(pcnt0!=-1) 
        if(nHits==0){
          cout<<"Warniing: Find a 0 hit event ? at eventID = "<<iData<<endl;
          iss<<"Warniing: Find a 0 hit event ? at eventID = "<<iData<<endl;
        }else if(iData!=0){
          eventID = iData-1;
          t->Fill();
        }else{
          ...
          iss<<"Warning: Get Start Filling the TTree from 1st event"<<endl;
          iss<<"And Clear the data before 1st event"<<endl;
          iss<<"Totally erase: "<<nHits<<" hits"<<endl;
        }
        iData++;
        bHit_.clear();
        HitGTMID_.clear();
        ...
        cout<<"Error: Can't find Header!!! Problem header: "<<Hitdata[0].data()<<endl;
        inRawSci>>Hitdata[0]>>Hitdata[1];
        cout<<"  Try: consume 2 bytes data to revise!"<<endl;
        iss<<"Error: Can't find Header!!! Problem header: "<<Hitdata[0].data()<<endl;
        inRawSci>>Hitdata[0]>>Hitdata[1];
        iss<<"  Try: consume 2 bytes data to revise!"<<endl;
        // throw;
      }
    }
  ...
  ofstream ott("/data4/YuSiang/LYSOReadOut/Logs/ConvertProblem.dat", ofstream::out | ofstream::app );
  ott<<"-----------------------------\n"<<endl;
